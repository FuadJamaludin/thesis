...
network.loads_t.p_set = pd.DataFrame(index=network.snapshots,
                                     ...
                                     data=np.random.rand(len(network.snapshots), len(network.loads)))

# scenario: AC loads only, WITHOUT H2 load/demand
# data=np.random.rand(len(network.snapshots), len(network.loads) - optimization successful
# data=10*np.random.rand(len(network.snapshots), len(network.loads) - optimization successful
# data=100*np.random.rand(len(network.snapshots), len(network.loads) - optimization failed

# scenario: AC loads + H2 load/demand
# data=np.random.rand(len(network.snapshots), len(network.loads) - optimization failed
# insufficient Gen capacity
